...
PRODUCT_CATEGORIES = {
    ...
    # ... (остальные категории остаются без изменений, сократил для краткости)
    ...
    # ... (другие категории опущены, добавьте их полностью)
}
...
# Загрузка и обработка данных (без виджетов)
@st.cache_data
        ...
        if file_type == 'csv':
            ...
            if 'data' in data and isinstance(data['data'], list):
                ...
                if response.status_code == 200:
                    ...
                    st.error(f"Ошибка API: {response.text} (код: {response.status_code})")
                    return pd.DataFrame()
            else:
